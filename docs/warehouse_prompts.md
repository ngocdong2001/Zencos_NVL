...
- base_unit        string, default 'GR'     (packaging dùng 'PCS')
- min_stock_level  DECIMAL(15,4), default 0
- notes            text, nullable
- SoftDeletes + timestamps

━━━ 3. product_units (quy cách — chỉ cho raw_material) ━━━
- id
- product_id       FK → products
- unit_name        string   VD: 'drum', 'kg', 'gói'
...
- hasMany(Batch)
- scopeRawMaterials(): lọc product_type = 'raw_material'
- scopePackaging(): lọc product_type = 'packaging'
...
━━━ 1. catalog_classifications (Phân loại NVL) ━━━
...
Trường products.category lưu catalog_classifications.code.

━━━ 2. catalog_units (Đơn vị tính) ━━━
- id         bigIncrements (unsigned BIGINT)
- code       string, unique   (VD: 'GR', 'KG', 'PCS', 'DRUM', 'GOI')
- name       string           (tên hiển thị: 'Gram', 'Kilogram', 'Cái/Chiếc', ...)
- notes      text, nullable
- status     string, default 'Active'
- deleted_at datetime, nullable  (soft delete)
- timestamps

Mục đích: Dropdown chọn đơn vị cơ sở khi tạo/sửa NVL/bao bì.
Liên kết với products.base_unit và product_units.unit_name.

━━━ 3. catalog_locations (Vị trí kho) ━━━
- id         bigIncrements (unsigned BIGINT)
...
GHI CHÚ TRIỂN KHAI:
- 3 bảng dùng chung cấu trúc BasicRow = { id, code, name, note, status }
...
products               | product_type: raw_material/packaging | SoftDeletes
product_units          | conversion_to_base DEC(15,4)
...
── CATALOG TABLES (3 bảng bổ sung — danh mục tra cứu) ─────────
catalog_classifications | Phân loại NVL: id, code(unique), name, notes, status, deleted_at, timestamps
catalog_units          | Đơn vị tính: id, code(unique), name, notes, status, deleted_at, timestamps
catalog_locations      | Vị trí kho: id, code(unique), name, notes, status, deleted_at, timestamps
